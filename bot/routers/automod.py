...
Runs on every text/caption message in groups where mod_enabled=True.
Applies link filter and keyword filter; warns then kicks on repeat offences.
"""
import json
import logging
import re
...
_URL_RE = re.compile(
    r"(https?://|t\.me/|@\w{5,}|"   # http links, t.me links, channel usernames
    r"[\w\-]{2,}\.(com|org|net|io|app|ru|uk|info|biz|xyz)\b)",
    re.IGNORECASE,
)

# Simple in-memory admin status cache: (chat_id, user_id) → (is_admin, expires_ts)
...
async def _warn_and_maybe_kick(
    ...
    if count >= warn_limit:
        ...
        try:
            ...
            await bot.unban_chat_member(chat_id, user.id)  # kick, not ban
        except TelegramBadRequest:
            pass
...
async def automod_filter(message: Message, bot: Bot, session: AsyncSession) -> None:
    ...
    # Admins and creators are exempt
