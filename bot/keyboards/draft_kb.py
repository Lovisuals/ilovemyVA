...
def build_step1_kb() -> InlineKeyboardMarkup:
    ...
    if url:
        ...
        builder.row(
            InlineKeyboardButton(text="Type Instead", callback_data=PostAction(action="type_mode").pack()),
            _cancel_btn(),
        )
    else:
...
def build_step2_kb() -> InlineKeyboardMarkup:
    ...
    builder.row(
        InlineKeyboardButton(text="Edit Subject", callback_data=PostAction(action="edit_subj").pack()),
        _cancel_btn(),
    )
    return builder.as_markup()
def build_action_kb() -> InlineKeyboardMarkup:
    ...
    if url:
        builder.row(
            ...
            InlineKeyboardButton(text="Subject",      callback_data=PostAction(action="edit_subj").pack()),
        )
    else:
        builder.row(
            InlineKeyboardButton(text="Edit Subject", callback_data=PostAction(action="edit_subj").pack()),
            InlineKeyboardButton(text="Edit Body",    callback_data=PostAction(action="edit_body").pack()),
        )
    builder.row(_cancel_btn())
    return builder.as_markup()
...
def build_multi_time_kb(selected_slots: List[str]) -> InlineKeyboardMarkup:
    ...
    for h in range(24):
        for m in [0, 30]:
            slot = f"{h:02d}{m:02d}"
            label = f"{h:02d}:{m:02d}"
            state = "[x]" if slot in selected_slots else "[ ]"
            builder.button(text=f"{state} {label}", callback_data=MultiTimeToggle(action="toggle", slot=slot).pack())
    builder.adjust(6)
...
def build_day_kb(selected: List[str]) -> InlineKeyboardMarkup:
    ...
    for label, key in _DAYS:
        state = "[x]" if key in selected else "[ ]"
        builder.button(text=f"{state} {label}", callback_data=DayToggle(day=key).pack())
    builder.adjust(4, 3)
    builder.row(
...
def build_target_kb(
    ...
    builder = InlineKeyboardBuilder()
    for chat in chats:
        state = "[x]" if chat.chat_id in selected_ids else "[ ]"
        type_label = "CH" if chat.chat_type == "channel" else "GP"
        builder.button(
            text=f"{state} {type_label} {chat.title}",
            callback_data=TargetToggle(action="chat", chat_id=chat.chat_id).pack(),
        )
    builder.adjust(1)
    builder.row(
        InlineKeyboardButton(text="All",  callback_data=TargetToggle(action="all",  chat_id=0).pack()),
