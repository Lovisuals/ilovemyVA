...
from bot.callbacks import BucketSelect, ItemView
...
def build_bucket_panel(
    ...
    for bucket_name, icon in buckets:
        ...
        builder.button(
            text=label,
            callback_data=BucketSelect(bucket=bucket_name)
        )
    ...
    for item in items:
        ...
        builder.row(
            builder.button(
                text=f"• {preview}",
                callback_data=ItemView(item_id=str(item.id))
            ).button
        )

    # Pagination Row
    if total_pages > 1:
        paginator = build_paginator_buttons(f"bucket_pg:{active_bucket}", page, total_pages)
