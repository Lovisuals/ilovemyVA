    ...
    for user in users:
        ...
        builder.row(
            builder.button(
                ...
                callback_data=UserAction(user_id=user.id, action="view")
            ).button
        )
        
    if total_pages > 1:
        paginator = build_paginator_buttons("usr_p", page, total_pages)
...
def build_user_actions(user_id: int, current_role: UserRole) -> InlineKeyboardMarkup:
    ...
    if current_role == UserRole.USER:
        builder.button(text="🛡 Promote", callback_data=UserAction(user_id=user_id, action="promote"))
    elif current_role == UserRole.ADMIN:
        builder.button(text="👤 Demote", callback_data=UserAction(user_id=user_id, action="demote"))
        
    builder.button(text="🚫 Remove", callback_data=UserAction(user_id=user_id, action="remove"))
    builder.button(text="← Back", callback_data=UserPage(page=1))
