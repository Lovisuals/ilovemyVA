...
def build_time_picker(item_id: str) -> InlineKeyboardMarkup:
    ...
    for t in times:
        builder.button(text=t, callback_data=ScheduleTime(item_id=item_id, time_str=t))
        
    builder.button(text="⌨️ Custom Time", callback_data=ScheduleCustom(item_id=item_id))
...
def build_recurrence_picker(item_id: str) -> InlineKeyboardMarkup:
    ...
    for label, val in options:
        builder.button(text=label, callback_data=ScheduleRecurrence(item_id=item_id, recurrence=val))
